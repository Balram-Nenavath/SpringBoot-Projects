...
import com.springjwt.demo.filter.JwtFilter;
...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter{
    ...
    public void configure(AuthenticationManagerBuilder authenticationManagerBuilder) throws Exception {
    	 authenticationManagerBuilder.inMemoryAuthentication().withUser("balram").password("{noop}balram@1998")
    	.roles("USER").and().withUser("admin").password("{noop}balram@1998").credentialsExpired(false)
    ...
    protected void configure(HttpSecurity http) throws Exception {
    	...
    	.disable()
   	.authorizeRequests()
    	.antMatchers("/health", "/actuator","/authenticate").permitAll()
